...
function mixinMigration(MsSQL) {
  ...
  MsSQL.prototype.getColumnsToAdd = function(model, actualFields) {
    ...
    function changed(newSettings, oldSettings) {
      ...
      if (oldSettings.Null === 'NO' && newSettings.hasOwnProperty('required') &&
        newSettings.required === false) {
        return true;
      }
      if (oldSettings.Type.toUpperCase().replace(/[^A-Z ]/g, "") !==
        datatype(newSettings).toUpperCase().replace(/[^A-Z ]/g, "")) {
        return true;
      }
      return false;
    }
  ...
  MsSQL.prototype.addIndexes = function(model, actualIndexes) {
    ...
    propNames.forEach(function(propName) {
      ...
      if (!found) {
        ...
        var name = propName + '_' + kind + '_' + type + '_' + rand + '_idx';
        if (i.name) {
          name = i.name;
        }
    ...
    for (var i = 0, n = props.length; i < n; i++) {
      ...
      if (prop === modelPKID) {
        ...
        if (idProp.type === Number) {
          ...
          if (idProp.generated !== false) {
            sql.push(self.columnEscaped(model, modelPKID) +
              ' [nvarchar](255) DEFAULT newid() NOT NULL');
          } else {
            sql.push(self.columnEscaped(model, modelPKID) + ' ' +
  ...
  MsSQL.prototype.singleIndexSettingsSQL = function(model, prop, add) {
    ...
    var name = prop + '_' + kind + '_' + type + '_' + rand + '_idx';
    if (i.name) {
      name = i.name;
    }
